<?php
  ...
  function include_layout_template($template = "", $data = array()) {
    extract($data);
    include(VIEW.'layouts'.DS.$template);
  }

  // Dynamically Initializing the navigation pane
  function navigation($pageName="") {
    $navigation_array = [ 'vehicles' => "Bsc" , 
                          'electronics-and-computer' => "Bcom",
                          'mobiles-and-tablets' => "BMS",
                          'clothing-and-accessories' => "BA",
                          'books-and-cds' => "BCA",
                          'home-and-furniture' => "MBA",
                          'other' => "other"  ];
    $output = "";
